...
Especificación de la clase Parking
Comparativa de implementación: arrays vs colecciones
...
Si ticketsAbiertos.containsKey(vehiculo.getMatricula()) → IllegalArgumentException.
Instancia new Ticket(vehiculo, fechaEntrada).
...
Ticket ticket = ticketsAbiertos.get(matricula) — si null, lanza IllegalArgumentException.
ticket.salir(salida, formaPago) — cierra el ticket.
ticketsAbiertos.remove(matricula) — elimina del mapa.
ticketsCerrados.add(ticket) — añade a la lista.
Actualiza acumuladores: totalSalidas, importeTotalFacturado y el acumulado de la forma de pago.
Devuelve el ticket cerrado.
